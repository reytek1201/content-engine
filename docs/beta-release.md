...
**Plugins requiring native rebuild after `cap:sync`:** Haptics, Keyboard, Camera, Media, Push, Biometric Auth, **RevenueCat (`PurchasesPlugin`)**, and others listed in `ios/App/App/capacitor.config.json` → `packageClassList`.

**Android IAP note:** `android/app/src/main/assets/capacitor.plugins.json` is **gitignored** and regenerated by `npm run cap:sync`. If you upload an AAB without syncing after adding RevenueCat, the app shows *"Purchases plugin is not implemented on Android"* on upgrade. Always run `npm run cap:sync` immediately before generating a signed bundle.
