  ...
  if (!readiness.connected) {
    helperText =
      readiness.canConnectPlatform === false
        ...
        : "Connect TikTok in Settings, then post your video.";
  } else if (!readiness.tierAllowed) {
    helperText =
      "Your plan allows publishing from one platform. Disconnect other accounts or upgrade to post to TikTok.";
  ...
  const panelBody = (
    <>
      ...
      {!readiness.connected && readiness.canConnectPlatform === false ? (
        ...
        <PlatformTierUpgradeNotice message="Publishing to TikTok requires upgrading or removing extra connected accounts in Settings." />
      ) : null}
